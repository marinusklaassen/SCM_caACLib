/*
 * FILENAME: SCMMultiStepView
 *
 * DESCRIPTION:
 *         Based on the sclang EZSlider, however it's a child of View and the child views are organised by a layout, so it can scale.
 *
 *         SCMMultiStepView(nil, 400@100, \db.asSpec, 0.5, "test").front().action_({ |v| v.postln; }).labelText_("Label test").value_(1.0.rand).value.postln;
 *         SCMMultiStepView()
 *
 * AUTHOR: Marinus Klaassen (2012, 2021Q3)
 *
 * EXAMPLE:
a = SCMMultiStepView().front().editMode_(true)
b = a.getState();
a.proxySteps
a.editMode = true;
a.randomize
