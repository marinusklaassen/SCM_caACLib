/*
 * FILENAME: RangeSliderView
 *
 * DESCRIPTION:
 *         Based on the sclang EZSlider, however it's a child of View and the child views are organised by a layout, so it can scale.
 *
 *         RangeSliderView(nil, 400@100, \db.asSpec, 0.5, "test").front().action_({ |v| v.postln; }).labelText_("Label test").value_(1.0.rand).value.postln;
 *         RangeSliderView()
 *
...
RangeSliderView : View {
	...
	*new { | parent, bounds, spec, initVal, labelText, uiMode |
	...
	init {
		...
		rangeSliderView = RangeSliderFactory.createInstance(this)
